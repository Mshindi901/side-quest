...
export const downloadReportPdf = async (req, res) => {
    try {
        ...
        const drawSection = (title, items, color) => {
            ...
            document.moveDown(0.35).fillColor('#e5e7eb').rect(document.x, document.y, pageWidth, 1).fill();
            ...
            items.forEach((item) => {
                ...
                document.fillColor('#f8fafc').roundedRect(document.x, rowY - 3, pageWidth, 22, 3).fill();
                document.fillColor('#111827').font('Helvetica').fontSize(10).text(item.name, document.x + 8, rowY + 3, { width: pageWidth - 150, lineBreak: false });
                document.fillColor(color).font('Helvetica-Bold').text(formatAmount(item.amount), document.x + pageWidth - 140, rowY + 3, { width: 132, align: 'right', lineBreak: false });
                document.y = rowY + 27;
            });
        };

        document.fillColor('#166534').font('Helvetica-Bold').fontSize(24).text(`Report for ${batchName}`);
        ...
        [
            ...
            const cardX = document.x + index * (cardWidth + 10);
